...
def ev_charging_actions_user(pyscript_app_config):
    ...
    ev_charging_action = annotations_df.query('dashboardId == 11 and panelId == 2').copy() # action panel ev

    # Verarbeite Annotation
    entity_id = "ev_charging_action_target_soc"
    measurement = "%"
    tags = {
            "state_class_str": "measurement",
            "entity_id": entity_id,
            "friendly_name": "EV Charging Action Target SOC",
            "source": "ev_charging_actions_user"
            }  
    datapoint_list = []

    for annotation in ev_charging_action.itertuples():
