...
def space_heating_actions_user(pyscript_app_config):
    ...
    isg_set_heizkreis = annotations_df.query('dashboardId == 7 and panelId == 15').copy() # action panel heizung

    # Verarbeite ISG Warmwasser Annotation
    entity_id = "isg_heizkreis_zieltemperatur"
    measurement = "°C"
    tags = {
            "state_class_str": "measurement",
            "entity_id": entity_id,
            "friendly_name": "ISG Heizkreis Zieltemperatur",
            "source": "pyscript.annotations_to_influxdb"
            }  
    datapoint_list = []

    for annotation in isg_set_heizkreis.itertuples():
